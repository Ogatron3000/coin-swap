...
export default function CopyButton({ coinName }) {
    const [isNotifVisible, setIsNotifVisible] = useState(false)

    function handleClick() {
        navigator.clipboard.writeText(coinName)
    ...
    return (
        <div className={styles.copyButtonWrapper}>
            <div className={styles.notification} style={{display: isNotifVisible ? 'block' : 'none'}}>
                Coin name copied
            </div>
            <button className={styles.copyButton} onClick={handleClick} aria-label="Copy coin name to clipboard">
